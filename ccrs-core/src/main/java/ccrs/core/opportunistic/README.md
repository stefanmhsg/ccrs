...
|
|-- hypermedea/                             # Replaceable Hypermedea HTTP/history provider
|   |-- CcrsHypermedeaArtifact.java         # Artifact wrapper.
...
|   |-- JasonInteractionLog.java            # InteractionHistoryProvider implementation.
|   `-- README.md
|
`-- resources/
...
* **[OpportunisticCcrs.java](OpportunisticCcrs.java)**: Defines the scanner contract. `scan(triple)` handles one RDF triple; `scanAll(triples)` handles a batch and enables structural matching.
* **[VocabularyMatcher.java](VocabularyMatcher.java)**: Main runtime component. It first discovers embedded CCRS vocabulary definitions in incoming batches, asks the vocabulary to integrate them, then delegates simple patterns to O(1)-style lookups and structural patterns to the fast or slow path engines.
* **[StructuralPatternMatcher.java](StructuralPatternMatcher.java)**: High-performance recursive matcher for compiled basic graph patterns. Avoids Jena overhead for simple structural patterns.
* **[ScoringStrategy.java](ScoringStrategy.java)**: Interface separating what is scored (priority and relevance) from how utility is calculated. Enables custom utility calculation via subclassing.
* **[DefaultScoringStrategy.java](DefaultScoringStrategy.java)**: Standard CCRS scoring with saturating normalization `x/(x+1)`. Handles simple patterns, structural patterns without relevance variables, and structural patterns with extracted relevance variables.
* **[RdfTriple.java](../rdf/RdfTriple.java)**: Minimal immutable RDF triple representation. Keeps the core independent from Jason, CArtAgO, and Jena-specific runtime objects.
* **[CcrsVocabulary.java](../rdf/CcrsVocabulary.java)**: Vocabulary model and compiler. Centralizes CCRS namespace constants, recognized definition predicates, compiled simple indexes, structural pattern compilation, validation, and runtime vocabulary integration.
* **[CcrsVocabularyLoader.java](../rdf/CcrsVocabularyLoader.java)**: Loads initial vocabulary sources from classpath resources, file paths, or HTTP(S) URLs. `loadDefault()` loads `classpath:ccrs-vocabulary.ttl`.

### JaCaMo Platform Adapters

**Jason Adapters** (`jacamo/jason/opportunistic`):

* **[CcrsAgent.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jason/opportunistic/CcrsAgent.java)**: Extends standard Jason `Agent`. Intercepts incoming perceptions in the belief revision function (BRF) and adds detected opportunities as beliefs.
* **[CcrsConfiguration.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jason/opportunistic/CcrsConfiguration.java)**: Java-side helper for configuring scanners and startup vocabularies.
* **[prioritize.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jason/opportunistic/prioritize.java)**: Internal action (`ccrs.prioritize/2`) that reorders hypermedia options (candidate links) based on CCRS utilities. It accesses the belief base to match options with `ccrs(Target, PatternType, Utility)` beliefs and preserves rich annotations in the detailed output for rich decision-making.

**Jason Utilities** (`jacamo/jason`):

* **[JasonRdfAdapter.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jason/JasonRdfAdapter.java)**: Converts Jason `rdf(S,P,O)` literals to [RdfTriple.java](../rdf/RdfTriple.java) and converts [OpportunisticResult.java](OpportunisticResult.java) to `ccrs/3` beliefs.
* **[TimestampedBeliefBase.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jason/TimestampedBeliefBase.java)**: Belief base implementation with temporal tracking.

**Hypermedea Instrumentation** (`hypermedea` / package `ccrs.hypermedea`):

* Provides transparent HTTP interaction logging via Java SPI without modifying the Hypermedea artifact.
* **[CcrsHttpBinding.java](../../../../../../../ccrs-hypermedea/src/main/java/ccrs/hypermedea/CcrsHttpBinding.java)**: Protocol binding discovered via SPI; intercepts HTTP operation creation.
* **[CcrsHttpOperation.java](../../../../../../../ccrs-hypermedea/src/main/java/ccrs/hypermedea/CcrsHttpOperation.java)**: Logging wrapper for HTTP operations.
* **[InteractionLogSink.java](../../../../../../../ccrs-hypermedea/src/main/java/ccrs/hypermedea/InteractionLogSink.java)**: Interface for interaction logging.
* **[JasonInteractionLog.java](../../../../../../../ccrs-hypermedea/src/main/java/ccrs/hypermedea/JasonInteractionLog.java)**: bounded interaction history provider installed through `CcrsJacamoRuntime`.
* See **[README.md](../../../../../../../ccrs-hypermedea/README.md)** for Hypermedea-specific architecture details.
...
* **[CcrsAgentArch.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jaca/CcrsAgentArch.java)**: Extends `CAgentArch`. Intercepts observable properties, forwards the standard path with `super`, buffers RDF triples per perception cycle, calls `scanAll(...)`, and injects derived `ccrs/3` beliefs.
...
The default vocabulary is loaded by [CcrsVocabularyLoader.java](../rdf/CcrsVocabularyLoader.java):

```java
CcrsVocabularyLoader.loadDefault()
```

This resolves `classpath:ccrs-vocabulary.ttl`, reads it as Turtle into a Jena `Model`, and constructs [CcrsVocabulary.java](../rdf/CcrsVocabulary.java). `load(String... sources)` can also merge multiple startup sources before compilation. Sources may be classpath resources, file paths, or HTTP(S) URLs.
...
Recognized definition predicates are centralized in [CcrsVocabulary.java](../rdf/CcrsVocabulary.java):
...
When [CcrsVocabulary.java](../rdf/CcrsVocabulary.java) is constructed or recompiled:

* **Simple Patterns** are indexed by `patternType + matchesPosition` for fast lookup.
* **Simple Structural Patterns** are parsed from `ccrs:sparqlPattern` and compiled to Java constraints handled by [StructuralPatternMatcher.java](StructuralPatternMatcher.java).
...
Runtime discovery is implemented inside [VocabularyMatcher.java](VocabularyMatcher.java), before normal matching in `scanAll(...)`. There is no agent-facing load action.
...
6. [CcrsVocabulary.java](../rdf/CcrsVocabulary.java) merges the new triples, compiles the merged candidate model first, and only updates the active vocabulary if compilation succeeds.
...
`scanAll(...)` in [VocabularyMatcher.java](VocabularyMatcher.java) runs in this order:
...
4. Score each result through [ScoringStrategy.java](ScoringStrategy.java).
5. Return a list of [OpportunisticResult.java](OpportunisticResult.java).
...
The core returns [OpportunisticResult.java](OpportunisticResult.java) values containing:
...
Platform adapters convert these results into agent-facing beliefs. In the JaCaMo path, [JasonRdfAdapter.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jason/JasonRdfAdapter.java) creates:
...
* **Jason:** [CcrsAgent.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jason/opportunistic/CcrsAgent.java) scans RDF percepts in the BRF path.
* **CArtAgO:** [CcrsAgentArch.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jaca/CcrsAgentArch.java) uses a short-lived buffer and schedules a flush at the `BeginOfNextCycle`, ensuring RDF properties from the current environmental update are processed together before reasoning starts.
...
* **Custom Scoring:** Implement [ScoringStrategy.java](ScoringStrategy.java) and inject it through `VocabularyMatcher.Factory`.
* **IDs:** URI resources are used directly; anonymous structures are identified by label or generated id in [CcrsVocabulary.java](../rdf/CcrsVocabulary.java).
...
* **CArtAgO visibility:** [CcrsAgentArch.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jaca/CcrsAgentArch.java) uses `super` calls and internal helpers to preserve standard CArtAgO behavior while adding CCRS interpretation.
...
+------+-------------------------------+--------+
       ...
       | via super.add...              |
       |                               | a. Convert ObsProperty to Literal
       v                               | b. Check if isRdfObservable()
+----------------------+               | c. Buffer into perceptionBatch
| CAgentArch (Legacy)  |               |    grouped by source URI
+------+---------------+               |
       |                               | 4. FLUSH at begin of next cycle
       | obsPropToLiteral()            |
       |                               v
       |                     +---------------------------------+
       ...
       |                               | 5. scanAll(batch)
       |                               |    discover vocabulary
       |                               |    integrate and recompile
       |                               |    match structural/simple patterns
       |                               v
       |                     +---------------------------------+
       |                     | JasonRdfAdapter                |
       |                     | ccrs.jacamo.jason              |
...
1. **Non-Blocking Interception:** [CcrsAgentArch.java](../../../../../../../ccrs-jacamo/src/main/java/ccrs/jacamo/jaca/CcrsAgentArch.java) allows the standard flow to proceed immediately, so the agent still receives the raw `rdf/3` perceptions.
...
3. **Core Isolation:** The interpretation logic happens in [VocabularyMatcher.java](VocabularyMatcher.java) and [CcrsVocabulary.java](../rdf/CcrsVocabulary.java). This layer knows nothing about Jason or CArtAgO.
